...
int main(){
  CImg<unsigned char> src("cup.jpeg");
  int width = src.width();
  int height = src.height();
  unsigned char* ptr = src.data(10,10,0); // get pointer to pixel @ 10,10,0
...
float *process() {
  ...
  CImg<double> src("cup.jpeg");
  int width = src.width();
  int height = src.height();
  double* ptr = src.data(10,10,0); // get pointer to pixel @ 10,10,0
  double pixel = *ptr;
  ...
  float *feat = (float *) calloc(1,sizeof(float));
