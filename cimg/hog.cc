...
// main function:
// takes a double color image
// returns HOG features
int main(int argc, char* argv[]) {
  
  if(argc!=2){
    cout << "Usage: " << argv[0] << " [jpg_file] " << endl;;
    return 0;
  }

  CImg<double> src(argv[1]);
  int width = src.width();
  ...
  int numBlocksY = out[0];
  int numBlocksX = out[1];
  int numFeaturesPerBlock = out[2];

  // for(int y=0; y<numBlocksY; y++){
  //     for(int x=0; x<numBlocksX; x++){
  //         for(int f = 0; f<numFeaturesPerBlock; f++){
  //             cout << std::setprecision(2) << feat[y + x*numBlocksY + f*numBlocksX*numBlocksY] << " "; 
  //             if(f==17 || f==26) cout << "  |  ";
  //         }
  //         cout << endl;
  //     }
